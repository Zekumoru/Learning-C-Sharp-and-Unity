	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			while (input[0] != 'q')
			{
				...
				// Create array for count
				int[] countsPerAlphabet = new int[26];

				// iterate through input
				for (int i = 0; i < input.Length; i++)
                {
					int index = 
						ConvertCharToIndex(Char.ToUpper(input[i]));
					if (index == -1) continue;
					countsPerAlphabet[index]++;
                }

				// output
				for (int i = 0; i < countsPerAlphabet.Length; i++)
                {
					int count = countsPerAlphabet[i];
					if (count == 0) continue;
